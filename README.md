...
To make AIDeck work as advertised such as mobile wifi FPV streamer, you need to fix some resource issues with GAP8. This repository will contain list of fixes needed to be done to make it work.

Starting from late 2025, Bitcraze decided to ship AI decks with wifi streamer example along with bootloader to make hardware testing easier. However, they flash outdated code, resulting in packet dropouts and crashes.

The following fixes are designed to update hardware to the latest software and set some configurations so that wifi streamer example is more solid and can be used for more applications

# Bootloader

If you AI Deck ships with wifi streamer example you dont need to flash bootloader. Thanks Bitcraze! (But you should have already done this 3 years ago).

# Prepare a wifi router

I noticed that Station mode of connecting the deck to wifi router and using ip address to access the ai deck feed is much more stable compared to the AP mode.

To change to station mode, refer to how to flash STM32 section. 

# Flashing ESP32

To flash ESP32 firmware to latest version, attach aideck to the crazyflie and flash latest cf2 firmware on it. The ESP32 firmware will be flashed automatically through this process.

# Flashing STM32 

Next you need to flash stm32 chip on the crazyflie itself to custom compiled firmware.

First gitclone repo for firmware.
```
git clone --recursive https://github.com/bitcraze/crazyflie-firmware.git
```
Install dependencies (Assuming fresh ubuntu 24 desktop)
```
sudo apt update
sudo apt install libncurses-dev
sudo apt install gcc-arm-none-eabi libnewlib-arm-none-eabi
```
Compile. Note that you have to go to expansion deck configuration and change wifi credentials after chaning to station mode not ap mode. Also note that wifi passwords must be 8 characters or more. After flashing you can check ip address of deck by going to cfclient console tab. 
```
make menuconfig
...
make -j 12
```
Flash. Change radio address as necessary.
```
